      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-[13px] leading-5 text-muted-foreground">
            {hasMarkdown
              ? '생성된 코드를 GitHub README에 붙여 넣어 카드를 표시할 수 있습니다.'
              : '현재 설정으로 GitHub README에 붙여 넣을 Markdown 코드를 준비합니다.'}
          </p>
        </div>
      ...
      {hasMarkdown ? (
        <div className="mt-5">
          ...
          {isFallbackMarkdown ? (
            <p
              ...
              role="status"
            >
              카드 저장 서버의 응답이 지연되어, 현재 설정을 바로 사용할 수 있는 호환 URL 코드로 생성했습니다. 나중에
              다시 생성하면 짧은 URL 코드로 바꿀 수 있습니다.
            </p>
          ) : null}
        ...
        <div className="mt-5 rounded-xl border border-dashed border-border bg-background/50 px-5 py-8 text-center">
          ...
          <p className="mt-1 text-[12px] leading-5 text-muted-foreground">
            커스텀을 마친 뒤 버튼을 눌러 카드를 저장하고 코드를 생성해 주세요.
          </p>
        </div>
      )}
    </section>
